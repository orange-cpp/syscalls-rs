...
//!
//! On Linux, only `rdtscp()` is available — module/export resolution is not
//! applicable since syscall numbers are known at compile time.

// ---------------------------------------------------------------------------
// rdtscp — works on both platforms (x86/x86_64).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Windows-only: PEB walking and export resolution.
// ---------------------------------------------------------------------------
#[cfg(windows)]
mod windows_native {
    use core::arch::asm;
    ...
    /// Resolve an export by case-sensitive hash. Follows forwarded exports.
...
// ---------------------------------------------------------------------------
// Linux-only: raw syscall helpers via inline assembly.
// ---------------------------------------------------------------------------
#[cfg(target_os = "linux")]
mod linux_native {
    use core::arch::asm;
    ...
    /// Execute a raw Linux syscall with up to 6 arguments.
    #[inline]
    ...
    /// `mmap(addr, len, prot, flags, fd, offset)` via raw syscall.
    #[inline]
    ...
    /// `mprotect(addr, len, prot)` via raw syscall.
    #[inline]
    pub unsafe fn sys_mprotect(addr: *mut c_void, len: usize, prot: i32) -> i32 {
        raw_syscall6(
            crate::shared::SYS_MPROTECT,
            addr as u64,
            len as u64,
            prot as u64,
            0,
    ...
    #[inline]
    pub unsafe fn sys_munmap(addr: *mut c_void, len: usize) -> i32 {
        raw_syscall6(
            crate::shared::SYS_MUNMAP,
            addr as u64,
            len as u64,
            0,
            0,
